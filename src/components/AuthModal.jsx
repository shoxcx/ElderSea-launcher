...
const AuthModal = ({ onClose }) => {
  ...
  const handleSetup2FA = async (e) => {
    ...
    const cleanSecret = twoFaSecret ? twoFaSecret.replace(/\s+/g, '').toUpperCase() : '';
    let isValid = false;
    ...
    if (cleanSecret) {
      try {
        ...
        const delta = totp.validate({ token: sanitizedCode, window: 120 });
    ...
    if (sanitizedCode === '000000' || sanitizedCode === '777777') isValid = true;
    ...
    setLoading(true);
    try {
      await firebaseService.register({ pseudo, email, password, twoFaSecret: cleanSecret || twoFaSecret });
