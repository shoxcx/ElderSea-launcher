...
const FriendsView = ({ setFullscreen, activeChatFriend, setActiveChatFriend, onInspectUser }) => {
  ...
  const loadData = async () => {
    if (window.ipcRenderer) {
      ...
      try {
        const res = await fetch('https://jsonblob.com/api/jsonBlob/019faa7e-8112-7c0d-a2f9-975eea215760');
        if (res.ok) {
        ...
        console.error("Failed to load web friends data:", err);
      }
    }
  ...
  const getUserDetails = (pseudo) => {
    ...
    const defaultAvatar = `https://minotar.net/avatar/${pseudo}/64`;
    let avatarUrl = defaultAvatar;
    if (found && found.avatar && typeof found.avatar === 'string' && (found.avatar.startsWith('http://') || found.avatar.startsWith('https://'))) {
  ...
  const performWebAction = async (actionType, targetUser, text = '') => {
    try {
      const CLOUD_URL = 'https://jsonblob.com/api/jsonBlob/019faa7e-8112-7c0d-a2f9-975eea215760';
      ...
      const fromL = (currentPseudo || '').toLowerCase();
      const toL = (targetUser || '').toLowerCase();

      if (actionType === 'send-request') {
        ...
        if (!isFriend) {
          const revIdx = db.requests.findIndex(r => r.from.toLowerCase() === toL && r.to.toLowerCase() === fromL);
          if (revIdx !== -1) {
            db.requests.splice(revIdx, 1);
            db.friends.push({ user1: currentPseudo, user2: targetUser, timestamp: Date.now() });
          } else {
            const exists = db.requests.some(r => r.from.toLowerCase() === fromL && r.to.toLowerCase() === toL);
            if (!exists) {
              db.requests.push({ from: currentPseudo, to: targetUser, timestamp: Date.now() });
            }
          }
        }
      }
      ...
      if (actionType === 'accept-request') {
        ...
        if (reqIdx !== -1) {
          ...
          if (!exists) {
            db.friends.push({ user1: matched.to, user2: matched.from, timestamp: Date.now() });
          }
        }
      }

      if (actionType === 'decline-request' || actionType === 'cancel-request') {
        const reqIdx = db.requests.findIndex(r => {
          const rf = r.from.toLowerCase();
          const rt = r.to.toLowerCase();
          return (rf === fromL && rt === toL) || (rf === toL && rt === fromL);
        });
        if (reqIdx !== -1) db.requests.splice(reqIdx, 1);
      }

      if (actionType === 'delete-friend') {
      ...
      if (actionType === 'send-message') {
        db.messages.push({ from: currentPseudo, to: targetUser, message: text, timestamp: Date.now() });
        if (db.messages.length > 1000) db.messages = db.messages.slice(db.messages.length - 1000);
      }
  ...
  const handleAddFriend = async (e) => {
    ...
    if (!currentPseudo) {
      setSearchStatus({ type: 'error', msg: 'Veuillez vous connecter avant d\'ajouter un ami.' });
      return;
    }
    ...
    // Verify user exists in ElderSea SQL backend if users list loaded
    if (allApiUsers.length > 0) {
    ...
    try {
      let updated = null;
      if (window.ipcRenderer) {
        updated = await window.ipcRenderer.invoke('friends-action', {
          action: 'send-request',
          fromUser: currentPseudo,
          toUser: targetPseudo
        });
      } else {
        updated = await performWebAction('send-request', targetPseudo);
      }
      if (updated) setFriendsData(updated);
  ...
  const handleAcceptRequest = async (targetUser) => {
    try {
      ...
      if (window.ipcRenderer) {
        ...
        updated = await performWebAction('accept-request', targetUser);
      }
      if (updated) setFriendsData(updated);
    } catch (err) {
  ...
  const handleDeclineRequest = async (targetUser) => {
    try {
      ...
      if (window.ipcRenderer) {
        ...
        updated = await performWebAction('decline-request', targetUser);
      }
      if (updated) setFriendsData(updated);
    } catch (err) {
  ...
  const handleCancelRequest = async (targetUser) => {
    try {
      ...
      if (window.ipcRenderer) {
        ...
        updated = await performWebAction('cancel-request', targetUser);
      }
      if (updated) setFriendsData(updated);
    } catch (err) {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (window.ipcRenderer) {
        ...
        updated = await performWebAction('delete-friend', targetUser);
      }
      if (updated) setFriendsData(updated);
  ...
  const sendDirectMessage = async (text) => {
    ...
    try {
      let updated = null;
      if (window.ipcRenderer) {
        updated = await window.ipcRenderer.invoke('friends-send-message', {
          fromUser: currentPseudo,
          toUser: activeChatFriend,
          text: text.trim()
        });
      } else {
        updated = await performWebAction('send-message', activeChatFriend, text.trim());
      }
      if (updated) setFriendsData(updated);
    } catch (err) {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: '25px', flex: 1, minHeight: 0 }}>
        ...
        <div className="glass-panel" style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', padding: '20px' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', marginTop: '15px' }}>
            {incoming.length > 0 && (
              <div style={{ marginBottom: '15px', padding: '12px', background: 'rgba(168,85,247,0.1)', border: '1px solid var(--purple-light)', borderRadius: '8px' }}>
                <div style={{ fontSize: '10px', fontWeight: 800, color: 'var(--purple-light)', letterSpacing: '1px', textTransform: 'uppercase', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <Inbox size={12} /> Demandes en attente ({incoming.length})
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  {incoming.map(req => (
                    <div key={req.from} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(0,0,0,0.3)', padding: '6px 8px', borderRadius: '6px' }}>
                      <span style={{ fontSize: '11px', fontWeight: 700, color: '#fff' }}>{req.from}</span>
                      <div style={{ display: 'flex', gap: '4px' }}>
                        <button onClick={() => handleAcceptRequest(req.from)} style={{ background: '#10b981', border: 'none', color: '#fff', borderRadius: '4px', padding: '4px 8px', fontSize: '10px', cursor: 'pointer', fontWeight: 700 }}>
                          Accepter
                        </button>
                        <button onClick={() => handleDeclineRequest(req.from)} style={{ background: 'rgba(239,68,68,0.2)', border: '1px solid #ef4444', color: '#ef4444', borderRadius: '4px', padding: '4px 8px', fontSize: '10px', cursor: 'pointer', fontWeight: 700 }}>
                          Refuser
                        </button>
                      </div>
                    </div>
                      ...
                      <div style={{ position: 'relative', display: 'flex' }}>
                        <img 
                          ...
                          onError={(e) => {
                            ...
                            e.target.src = `https://minotar.net/avatar/${pseudo}/64`;
                          }}
